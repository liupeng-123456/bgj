...
namespace AutoTF
{
    public  static class DbConfigManager
    {
        ...
        private static string ReadConnectionStr()
        {
           ...
           /*     if (SystemParams.Instance.DbConfigTypeEnum== SystemParams.DBConfigType.测试环境)
                {
                    fileName = "dbConfig-Test.json";
                }
                else if (SystemParams.Instance.DbConfigTypeEnum == SystemParams.DBConfigType.生产环境)
                {
                    fileName = "dbConfig-Dev.json";
                }
                else
                {
