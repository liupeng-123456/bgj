...
/*        [field: NonSerialized]
        private DBConfigType _dbConfigType ;

        [Permission(3), ReadOnly(false)]
        [DisplayName("0.数据库配置"), Category("1.数据库配置选择"), Description("数据库的连接配置信息 生产环境和开发环境...")]
        public DBConfigType DbConfigTypeEnum
        {
            get { return _dbConfigType; }
            set
            {
                if (_dbConfigType != value)
                {
                    _dbConfigType = value;
                    DbConfigManager.ReflashDBConfig();
                }
            }
        }*/
        ...
        [Permission(3), ReadOnly(false)]
        [DisplayName("1.消毒时间 单位(毫秒ms)"), Category("2.消毒配置"), Description("设置消毒时间")]
        //[Browsable(false)]
